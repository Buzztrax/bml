...
void bml(test_process(char *libpath,const char *infilename,const char *outfilename)) {
  ...
  */
  ...
  if((bm=bml(new(libpath)))) {
    ...
    bml(init(bm,0,NULL));
    bml(get_machine_info(bm,BM_PROP_TYPE,&mtype));
    bml(get_machine_info(bm,BM_PROP_FLAGS,&mflags));
    printf("  %s initialized\n",type_name[mtype]);
    bml(get_machine_info(bm,BM_PROP_MIN_TRACKS,(void *)&tracks));
    if(tracks) {
    ...
    if(infile && outfile) {
      ...
      if(mtype==1) {
        ...
        bml(get_machine_info(bm,BM_PROP_NUM_GLOBAL_PARAMS,&num));
        // set value for trigger parameter(s)
        for(i=0;i<num;i++) {
          bml(get_global_parameter_info(bm,i,BM_PARA_FLAGS,(void *)&pflags));
          if(!(pflags&2)) {
            bml(get_global_parameter_info(bm,i,BM_PARA_TYPE,(void *)&ptype));
            switch(ptype) {
              case 0: // note
              ...
              case 2: // byte
              case 3: // word
                bml(get_global_parameter_info(bm,i,BM_PARA_NO_VALUE,(void *)&pmaval));
        ...
        if(tracks) {
          bml(get_machine_info(bm,BM_PROP_NUM_TRACK_PARAMS,&num));
          for(i=0;i<num;i++) {
            bml(get_track_parameter_info(bm,i,BM_PARA_FLAGS,(void *)&pflags));
            if(!(pflags&2)) {
              bml(get_track_parameter_info(bm,i,BM_PARA_TYPE,(void *)&ptype));
              switch(ptype) {
                case 0: // note
                ...
                case 2: // byte
                case 3: // word
                  bml(get_track_parameter_info(bm,i,BM_PARA_NO_VALUE,(void *)&pmaval));
          ...
          for(i=0;i<num;i++) {
            ...
            if(!(pflags&2)) {
              ...
              switch(ptype) {
                ...
                case 2: // byte
                case 3: // word
                  bml(get_global_parameter_info(bm,i,BM_PARA_NO_VALUE,(void *)&pnoval));
            ...
            for(i=0;i<num;i++) {
              ...
              if(!(pflags&2)) {
                ...
                switch(ptype) {
                  ...
                  case 2: // byte
                  case 3: // word
                    bml(get_track_parameter_info(bm,i,BM_PARA_NO_VALUE,(void *)&pnoval));
