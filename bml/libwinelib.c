...
setup_signal (int sig)
{
	...
	if (sigaction (sig, &sigact, NULL)) {
		bml_error ("could not install signal handler for signal %d", sig);
	}
}
...
static DWORD WINAPI new_thread_proxy (LPVOID param)
{
	...
	if (pipe (proxy_pripe_request)) {
		bml_error ("cannot set up proxy request pipe (%s)", strerror (errno));
		return -1;
	}

	if (pipe (proxy_pripe_status)) {
		bml_error ("cannot set up proxy reply pipe (%s)", strerror (errno));
		return -1;
	}
	...
	while (1) {
		...
		if (read (proxy_pripe_request[0], &sri, sizeof (sri)) != sizeof (sri)) {
			bml_error ("cannot read new thread request from proxy request pipe (%s)", strerror (errno));
			return -1;
		}
		...
		if (write (proxy_pripe_status[1], &sri, sizeof (sri)) != sizeof (sri)) {
			bml_error ("cannot write request/reply back to proxy reply pipe (%s)", strerror (errno));
			/* XXX free teb */
			return -1;
		}
		
		if (write (proxy_pripe_status[1], &fd, sizeof (fd)) != sizeof (status)) {
			bml_error ("cannot write status back to proxy reply pipe (%s)", strerror (errno));
			/* XXX free teb */
			return -1;
		}

		if (write (proxy_pripe_status[1], &thread_info, sizeof (thread_info)) != sizeof (thread_info)) {
			bml_error ("cannot write thread info to proxy reply pipe (%s)", strerror (errno));
			/* XXX free teb */
			return -1;
		}
...
SharedWineInit(void (*sighandler)(int,siginfo_t*,void*))
{
	...
	if (setjmp (jump) == 0) {
		...
		if (Error[0]!='\0') {
			bml_error ("Wine initialization error:%s\n", Error);
			return -1;
		}
	}
	...
	*/
	...
	if (WineLoadLibrary ("user32.dll") == NULL) {
		bml_error ("cannot load Windows DLL user32");
		return 1;
	}

	if (WineLoadLibrary ("comdlg32.dll") == NULL) {
		bml_error ("cannot load Windows DLL comdlg32");
		return 1;
	}

	if (WineLoadLibrary ("oleaut32.dll") == NULL) {
		bml_error ("cannot load Windows DLL oleaut32");
		return 1;
	}
...
#define GET_SYMBOL(res,handle,name) if ((res = WineGetProcAddress (handle,name)) == NULL) { bml_error ("cannot find Windows function %s", name); return -1; }
	
	if ((ntdll = WineLoadLibrary( "ntdll.dll")) == NULL) {
		bml_error ("cannot load Windows DLL ntdll");
		return 1;
	}
	...
	if ((ntso = dlopen (ntdllpath, RTLD_NOW|RTLD_GLOBAL)) == NULL) {
		bml_error ("cannot open NT DLL (%s)", ntdllpath);
		return -1;
	}

	server_call = dlsym (ntso, "wine_server_call");
	if ((dlerr = dlerror ()) != NULL) {
		bml_error ("cannot find wine_server_call (%s)", dlerr);
		return -1;
	}
	send_fd = dlsym (ntso, "wine_server_send_fd");
	if ((dlerr = dlerror ()) != NULL) {
		bml_error ("cannot find wine_server_send_fd (%s)", dlerr);
		return -1;
	}
	init_thread = dlsym (ntso, "server_init_thread");
	if ((dlerr = dlerror ()) != NULL) {
		bml_error ("cannot find server_init_thread (%s)", dlerr);
		return -1;
	}
...
wine_adopt_thread (void)
{
	...
	{
		bml_error ("cannot read/write proxy thread request (%s)", strerror (errno));
	}
